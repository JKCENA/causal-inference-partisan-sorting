...
# (c) Obtain a heteroskedasticity‐robust covariance matrix
#     Type = "HC3" is a common choice; you can also use "HC0", "HC1", etc.
robust_vcov <- vcovHC(ipw.model_stab, type = "HC3") 

# (d) Get the robust standard error for sorted_2016
se_robust <- sqrt(robust_vcov["sorted_2016", "sorted_2016"])

# (e) Extract the point estimate
beta_hat <- coef(ipw.model_stab)["sorted_2016"]

# (f) Degrees of freedom (residual df from the lm fit)
df_resid <- df.residual(ipw.model_stab)

# (g) Critical t‐value for 95% CI
t_crit <- qt(0.975, df = df_resid)

# (h) Compute endpoints of the robust 95% CI
...
n    <- nrow(df_trim)
ate  <- ate_aipw                     # your AIPW point estimate
sdev <- sd(df_trim$aipw)            # sample SD of the aipw “scores”
se   <- sdev / sqrt(n)              # standard error of the mean
...
# 7. Calculate the E‐value using evalues.OLS()
#    This returns both the E‐value for the point estimate and the E‐value 
#    for the lower 95% CI bound.
...
# 8. Print out the E‐value results
print(e_val_results)
#========================================================
# 7) 부트스트랩으로 불확실성 추정 (선택사항)
#========================================================
library(sandwich)
dr_ate_stab <- function(data, indices) {
  d <- data[indices, ]  # bootstrap sample
  
  #### 1) Propensity score 모델
  ...
  #### 2) Stabilized IPTW 가중치 계산
  pA <- mean(d$sorted_2016)  # 전체 처치 비율
  d$stab_ipw <- ifelse(d$sorted_2016 == 1,
                       pA / d$ps,
                       (1 - pA) / (1 - d$ps))
  
  #### 3) Outcome regression (parametric g-formula)
